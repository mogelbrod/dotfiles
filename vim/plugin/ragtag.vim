...
inoremap <silent> <Plug>ragtagHtmlComplete <C-R>=<SID>htmlEn()<CR><C-A><C-O><C-P><C-R>=<SID>htmlDis()<CR><C-N>
...
function! s:Init()
  ...
  elseif exists("+omnifunc")
    inoremap <silent> <buffer> <SID>doctype  <C-R>=<SID>htmlEn()<CR><!DOCTYPE<C-A><C-O><C-P><C-R>=<SID>htmlDis()<CR><C-N><C-R>=<SID>doctypeSeek()<CR>
  elseif s:subtype() == "xhtml"
    imap <script> <buffer> <SID>doctype <SID>xhtmltrans
  else
    imap <script> <buffer> <SID>doctype <SID>htmltrans
  endif
  imap <script> <buffer> <C-A>! <SID>doctype

  imap <silent> <buffer> <C-A># <meta http-equiv="Content-Type" content="text/html; charset=<C-R>=<SID>charset()<CR>"<C-R>=<SID>closetag()<CR>
  inoremap <silent> <buffer> <SID>HtmlComplete <C-R>=<SID>htmlEn()<CR><C-A><C-O><C-P><C-R>=<SID>htmlDis()<CR><C-N>
  imap     <buffer> <C-A>H <SID>HtmlComplete
  inoremap <silent> <buffer> <C-A>$ <C-R>=<SID>javascriptIncludeTag()<CR>
  inoremap <silent> <buffer> <C-A>@ <C-R>=<SID>stylesheetTag()<CR>
  inoremap <silent> <buffer> <C-A><Space> <Esc>ciw<Lt><C-R>"<C-R>=<SID>tagextras()<CR>></<C-R>"><Esc>b2hi
  inoremap <silent> <buffer> <C-A><CR> <Esc>ciw<Lt><C-R>"<C-R>=<SID>tagextras()<CR>><CR></<C-R>"><Esc>O
  if exists("&omnifunc")
    inoremap <silent> <buffer> <C-A>/ <Lt>/<C-R>=<SID>htmlEn()<CR><C-A><C-O><C-R>=<SID>htmlDis()<CR><C-F>
    if exists(":XMLns")
      XMLns xhtml10s
    endif
  else
    inoremap <silent> <buffer> <C-A>/ <Lt>/><Left>
  endif
  let g:surround_{char2nr("p")} = "<p>\n\t\r\n</p>"
  let g:surround_{char2nr("d")} = "<div\1div: \r^[^ ]\r &\1>\n\t\r\n</div>"
  imap <buffer> <C-A><C-_> <C-A>/
  imap <buffer> <SID>ragtagOopen    <C-A><Lt><Space>
  imap <buffer> <SID>ragtagOclose   <Space><C-A>><Left><Left>
  if &ft == "php"
    inoremap <buffer> <C-A><Lt> <?php
    inoremap <buffer> <C-A>>    ?>
    inoremap <buffer> <SID>ragtagOopen    <?php<Space>print<Space>
    let b:surround_45 = "<?php \r ?>"
    let b:surround_61 = "<?php print \r ?>"
  elseif &ft == "htmltt" || &ft == "tt2html"
    inoremap <buffer> <C-A><Lt> [%
    inoremap <buffer> <C-A>>    %]
    let b:surround_45  = "[% \r %]"
    let b:surround_61  = "[% \r %]"
  ...
  imap     <buffer> <C-A>= <SID>ragtagOopen<SID>ragtagOclose<Left>
  imap     <buffer> <C-A>+ <C-V><NL><Esc>I<SID>ragtagOopen<Space><Esc>A<Space><SID>ragtagOclose<Esc>F<NL>s
  " <%\n\n%>
  if &ft == "cf"
    inoremap <buffer> <C-A>] <cfscript><CR></cfscript><Esc>O
  elseif &ft == "mason"
    inoremap <buffer> <C-A>] <%perl><CR></%perl><Esc>O
  elseif &ft == "html" || &ft == "xhtml" || &ft == "xml"
    imap     <buffer> <C-A>] <script<Space>type="text/javascript"><CR></script><Esc>O
  else
    imap     <buffer> <C-A>] <C-A><Lt><CR><C-A>><Esc>O
  endif
  " <% %>
  if &ft == "eruby"
    inoremap  <buffer> <C-A>- <%<Space><Space>-%><Esc>3hi
    inoremap  <buffer> <C-A>_ <C-V><NL><Esc>I<%<Space><Esc>A<Space>-%><Esc>F<NL>s
  elseif &ft == "cf"
    inoremap  <buffer> <C-A>- <cf><Left>
    inoremap  <buffer> <C-A>_ <cfset ><Left>
  else
    imap      <buffer> <C-A>- <C-A><Lt><Space><Space><C-A>><Esc>2hi
    imap      <buffer> <C-A>_ <C-V><NL><Esc>I<C-A><Lt><Space><Esc>A<Space><C-A>><Esc>F<NL>s
  endif
  " Comments
  if &ft =~ '^asp'
    imap     <buffer> <C-A>'     <C-A><Lt>'<Space><Space><C-A>><Esc>2hi
    imap     <buffer> <C-A>"     <C-V><NL><Esc>I<C-A><Lt>'<Space><Esc>A<Space><C-A>><Esc>F<NL>s
    let b:surround_35 = maparg("<C-A><Lt>","i")."' \r ".maparg("<C-A>>","i")
  elseif &ft == "jsp"
    inoremap <buffer> <C-A>'     <Lt>%--<Space><Space>--%><Esc>4hi
    inoremap <buffer> <C-A>"     <C-V><NL><Esc>I<%--<Space><Esc>A<Space>--%><Esc>F<NL>s
    let b:surround_35 = "<%-- \r --%>"
  elseif &ft == "cf"
    inoremap <buffer> <C-A>'     <Lt>!---<Space><Space>---><Esc>4hi
    inoremap <buffer> <C-A>"     <C-V><NL><Esc>I<!---<Space><Esc>A<Space>---><Esc>F<NL>s
  ...
  elseif &ft == "html" || &ft == "xml" || &ft == "xhtml"
    inoremap <buffer> <C-A>'     <Lt>!--<Space><Space>--><Esc>3hi
    inoremap <buffer> <C-A>"     <C-V><NL><Esc>I<!--<Space><Esc>A<Space>--><Esc>F<NL>s
    let b:surround_35 = "<!-- \r -->"
  elseif &ft == "django"
    inoremap <buffer> <C-A>'     {#<Space><Space>#}<Esc>2hi
    inoremap <buffer> <C-A>"     <C-V><NL><Esc>I<C-A>{#<Space><Esc>A<Space>#}<Esc>F<NL>s
    let b:surround_35 = "{# \r #}"
  else
    imap     <buffer> <C-A>'     <C-A><Lt>#<Space><Space><C-A>><Esc>2hi
    imap     <buffer> <C-A>"     <C-V><NL><Esc>I<C-A><Lt>#<Space><Esc>A<Space><C-A>><Esc>F<NL>s
  ...
  imap <buffer> <C-A>%           <Plug>ragtagUrlEncode
  imap <buffer> <C-A>&           <Plug>ragtagXmlEncode
...
if exists("g:ragtag_global_maps")
  imap     <C-A>H      <Plug>ragtagHtmlComplete
  imap     <C-A>/    </<Plug>ragtagHtmlComplete
  imap     <C-A>%      <Plug>ragtagUrlEncode
  imap     <C-A>&      <Plug>ragtagXmlEncode
